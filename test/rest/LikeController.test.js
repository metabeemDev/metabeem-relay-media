    ...
    describe("Query one", () =>
    {
        it("should return a record by hexId", async () =>
        {
            ...
            const response = await request(app)
                ...
                .send({
                    wallet : walletObj.address, data : { by : 'hexId', hexId : savedLike._id }, sig : ''
                });
            expect(response).toBeDefined();
            expect(response).toHaveProperty('statusCode');
            expect(response).toHaveProperty('_body');
        ...
        it("should return a record by hash", async () =>
        {
            ...
            const response = await request(app)
                .post('/v1/like/queryOne')
                .send({
                    wallet : walletObj.address, data : { by : 'hash', hash : savedLike.hash }, sig : ''
                });
            expect(response).toBeDefined();
            expect(response).toHaveProperty('statusCode');
            expect(response).toHaveProperty('_body');
            ...
            const response = await request(app)
                ...
                .send({
                    wallet : walletObj.address, data : {
                        by : 'walletAndRefTypeAndRefHash', refType : ERefDataTypes.post, refHash : savedPost.hash
                    }, sig : ''
                });
            expect(response).toBeDefined();
            expect(response).toHaveProperty('statusCode');
    ...
    describe("Deletion", () =>
    {
        it(`should logically delete a record by hash`, async () =>
        {
            ...
            const updateResponse = await request(app)
                .post('/v1/like/delete')
                .send({
                    wallet : walletObj.address, data : toBeDeleted, sig : toBeDeleted.sig
                });
            expect(updateResponse).toBeDefined();
            expect(updateResponse).toHaveProperty('statusCode');
            expect(updateResponse).toHaveProperty('_body');
